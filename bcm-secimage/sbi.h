...
typedef struct AuthenticatedHeader_t {
	...
	uint16_t SBIConfiguration;				/* Indicates SBI configuration */
...
#define CID_TYPE_PROD					(0x6 << 28) 	// (0110) Indicates that the device is a production SBI
#define CID_TYPE_DEV					(0x9 << 28)		// (1001) Indicates that the device is a development SBI
#define CID_TYPE_MASK					(0xF << 28)		// Mask for checking CID Type
...
int SBIAddBootLoaderPayload(SBI_HEADER *header, int verbose, char *filename, char *ivfilename, uint8_t big_endian, int alignByte, uint32_t sbiLoadAddress);
