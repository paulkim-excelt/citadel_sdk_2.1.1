 ...
 * tick() will call z_clock_announce() as many times.
 */
...
int z_clock_driver_init(struct device *dev)
{
	...
	arch_timer_set_cntfrq(CYC_PER_TICK);

	/* Enable timer interrupt */
	IRQ_CONNECT(PPI_IRQ(ARM_TIMER_PPI_IRQ), 0,
		tick, (void *)CYC_PER_TICK, 0);

	arch_counter_set_cntp_cval(CYC_PER_TICK);
...
u32_t z_clock_elapsed(void)
{
	...
	return arch_counter_get_cntpct() / CYC_PER_TICK;
}
