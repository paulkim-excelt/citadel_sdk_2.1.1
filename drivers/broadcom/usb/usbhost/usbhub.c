...
const usb_driver_t usbhub_driver = {
	"USB Hub",
	usbhub_attach,
	usbhub_detach
};

const usb_driver_t usbroothub_driver = {
	"Root Hub",
	usbhub_attach,
	usbhub_detach
};
...
static usbhub_softc_t softc_hub;
...
static int usbhub_attach(usbdev_t *dev, usb_driver_t *drv)
{
	 ...
	 */
	printf("USB: usbhub_attach\n");

	dev->ud_drv = drv;

	softc = &softc_hub;
	memset((void *)softc, 0, sizeof(usbhub_softc_t));
...
static int usbhub_detach(usbdev_t *dev)
{
	...
	for (idx = 0; idx < UHUB_MAX_DEVICES; idx++) {
		...
		if (deldev) {
			if (usb_noisy > 0)
				printf("USB: Removing device attached to bus %d hub %d port %d\n",
				       dev->ud_bus->ub_num,
			...
			if (deldev->ud_drv) {
				...
				if (usb_noisy > 0) {
					printf("USB: Detached device on bus %d hub %d port %d has no methods\n",
					       dev->ud_bus->ub_num,
					       dev->ud_address, idx+1);
				}
			}
...
static void usbhub_map_tree1(usbdev_t *dev, int level,
			     int (*func)(usbdev_t *dev, void *arg),
			     void *arg)
{
	usbhub_softc_t *hub;
	int idx;
...
void usbhub_map_tree(usbbus_t *bus, int (*func)(usbdev_t *dev,
		     void *arg), void *arg)
{
	usbhub_map_tree1(bus->ub_roothub, 0, func, arg);
}

void usbhub_map_from_device(usbdev_t *dev, int (*func)(usbdev_t *dev,
			    void *arg), void *arg)
{
...
void usbhub_reset_device(usbdev_t *dev, int port,
			 usb_port_status_t *portstatus)
{
	int i;

	if (usb_noisy > 0)
		printf("USB: Resetting device on bus %d port %d\n",
...
static void usbhub_scan_ports(usbdev_t *dev, void *arg)
{
	...
	usb_port_status_t portstatus_std_s;
	usb_port_status_t *portstatus_std = addr2uncached(&portstatus_std_s);

	if (!IS_HUB(dev))
		return; /* should not happen.  */

	portstatus = (usb_port_status_t *)portstatus_std;
	if (portstatus == NULL)
		return;
	...
	if (!(current & USB_PORT_STATUS_POWER)) {
		if (usb_noisy > 1)
			printf("USB: Powering up bus %d port %d\n",
	...
	for (idx = 0; idx < softc->uhub_nports; idx++) {
		...
		if (changed & USB_PORT_STATUS_RESET)
			/*FIXME usbhub_clear_port_feature ? */

		if (changed & USB_PORT_STATUS_OVERCUR) {
#ifdef _ESD_DEBUG_
			printf("hub %d port %d overcurrent\n", dev->ud_bus->ub_num, idx+1);
#endif
			usbhub_reset_device(dev, idx+1, portstatus);
			current = GETUSBFIELD(portstatus, wPortStatus);
			changed = GETUSBFIELD(portstatus, wPortChange);
#ifdef _ESD_DEBUG_
			printf("USB: Explore-overcurrent: Bus %d Hub %d port %d status %04X changed %04X\n",
		...
		if (changed & USB_PORT_STATUS_CONNECT) {
			...
			/* If disconnect, or (connect & something already there),
			 * then disconnect
			 */
			if (do_disconnect) {
				/*
				 ...
				 */
				printf("USBH: Device disconnected from bus %d hub %d port %d\n",
				       dev->ud_bus->ub_num,
				       dev->ud_address, idx+1);
				...
				if (newdev) {
					if (newdev->ud_drv) {
						...
						if (usb_noisy > 0) {
							printf("USB: Detached device on bus %d hub %d port %d "
							"has no methods\n",
							       dev->ud_bus->ub_num,
							       dev->ud_address, idx+1);
						}
			...
			if (do_connect) {
				...
				printf("USB: New device connected to bus %d hub %d port %d\n",
				       dev->ud_bus->ub_num,
				       dev->ud_address, idx+1);
				...
				res = usb_get_config_descriptor(newdev, (usb_config_descr_t *)buf,
								0, len);
				if (res != len)
