...
} __attribute__((aligned(ALIGNMENT))) ohci_ed_t; //make each object 64 byte sized
 ...
 */
#if 0
typedef struct ohci_itd_s { /*  HostController Iso Transfer Descriptor */
	volatile u32_t itd_control;	/*  xfer descriptor control */
	volatile u32_t itd_bp0;	/*  Buffer Page 0 */
	volatile u32_t itd_next_itd;	/*  Phys pointer to next xfer Desc */
	volatile u32_t itd_be;	/*  buffer End */
	volatile u32_t itd_offsetPSW10;	/*  Offset1/PSW1 Offset0/PSW0 */
	volatile u32_t itd_offsetPSW32;	/*  Offset3/PSW3 Offset2/PSW2 */
	volatile u32_t itd_offsetPSW54;	/*  Offset5/PSW5 Offset4/PSW4 */
	volatile u32_t itd_offsetPSW76;	/*  Offset7/PSW7 Offset6/PSW6 */
} ohci_itd_t;
#endif
...
#define OHCI_ITD_MAX_FC     8
#define OHCI_ITD_MAX_TDC    4

/*
 *  Unified Transfer Descriptor
 */
#define OHCI_TYPE_TD  1
#define OHCI_TYPE_ITD 2

typedef struct ohci_utd_s {
	union {
		ohci_td_t td;
		ohci_itd_t itd;
	} u;
	...
	volatile u32_t reserve[6];
} ohci_utd_t;	/*  64 bytes alignment */
#endif
 ...
 *  Registers
 */
#define USB_OHCI_BASE 0x46000800
...
#define R_OHCI_RHPORTSTATUS(x)	_OHCI_REGIDX(20+(x)) /* 1-based! */
