...
#ifndef ZBAR_TEST_IMG_WIDTH
#define ZBAR_TEST_IMG_WIDTH    (640)
#define ZBAR_TEST_IMG_HEIGHT   (480)
#endif
...
/** QRcode decode from a Y800 image data array */
extern int zbarimg_scan_one_y8_image(unsigned char *y8_img_buff, unsigned int width, unsigned int height, char *output, unsigned int *output_count);


/** QRcode decode from a YUYV image data array */
extern int zbarimg_scan_one_yuv422_image(unsigned char *YUY2_buff, unsigned int width, unsigned int height, char *output, unsigned int *output_count);


#ifdef CONFIG_ZBAR_DEBUG
#include <misc/printk.h>
#define ZBAR_PRINT(format, ...)         do{printk("%s():%d "format, __FUNCTION__, __LINE__, ##__VA_ARGS__);}while(0)
#define ZBAR_PRINT_LINE_NO()            do {ZBAR_PRINT("\n");}while(0)
#define ZBAR_RETURN_ON_NULL(x, ret)     if(!(x)){ ZBAR_PRINT("\n"); return (ret);}
#else
#define ZBAR_PRINT(format, ...)
#define ZBAR_PRINT_LINE_NO()
#define ZBAR_RETURN_ON_NULL(x, ret)
#endif
...
#include <kernel.h>
#undef ZBAR_MALLOC
#define ZBAR_MALLOC       k_malloc
#undef ZBAR_CALLOC
#define ZBAR_CALLOC       k_calloc
#undef ZBAR_REALLOC
#define ZBAR_REALLOC      k_realloc
#undef ZBAR_FREE
#define ZBAR_FREE         k_free
