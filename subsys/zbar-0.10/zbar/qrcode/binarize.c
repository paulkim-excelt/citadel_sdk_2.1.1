...
static unsigned char qr_binarize_mask_buffer[640*480]={0};
#endif
unsigned char *qr_binarize(const unsigned char *_img,int _width,int _height){
  ...
  if(_width>0&&_height>0){
...
#ifdef CONFIG_ZBAR_USE_STATIC_BUFFER
    mask=(unsigned char *)qr_binarize_mask_buffer;
#else
    mask=(unsigned char *)ZBAR_MALLOC(_width*_height*sizeof(*mask));
    ZBAR_RETURN_ON_NULL(mask, NULL);
#endif
    /*We keep the window size fairly large to ensure it doesn't fit completely
       inside the center of a finder pattern of a version 1 QR code at full
       resolution.*/
    ...
    windh=1<<logwindh;
    col_sums=(unsigned *)ZBAR_MALLOC(_width*sizeof(*col_sums));
    ZBAR_RETURN_ON_NULL(col_sums, NULL);
    /*Initialize sums down each column.*/
    for(x=0;x<_width;x++){
      g=_img[x];
      col_sums[x]=(g<<logwindh-1)+g;
