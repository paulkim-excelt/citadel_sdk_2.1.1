...
#include <arch/cpu.h>
#include <pinmux.h>
#include <broadcom/drivers/dmu.h>
...
#define CRMU_OVERRIDE_GPIO4		(6)
#define CRMU_OVERRIDE_UART_RX		(0)
 ...
 */
static int citadel_svk_58201_pinmux_init(struct device *arg)
{
...
#endif
#ifdef CONFIG_BRCM_DRIVER_GPIO
	struct device *dmu = device_get_binding(CONFIG_DMU_DEV_NAME);
	u32_t value;

	/* Set AON_GPIO 4 and 5 pins to GPIO by default */
	value = dmu_read(dmu, CIT_CRMU_IOMUX_CONTROL);
	value &= ~BIT(CRMU_OVERRIDE_GPIO4);
	value |= BIT(CRMU_OVERRIDE_UART_RX);
	dmu_write(dmu, CIT_CRMU_IOMUX_CONTROL, value);
#endif
...
SYS_INIT(citadel_svk_58201_pinmux_init, PRE_KERNEL_1,
