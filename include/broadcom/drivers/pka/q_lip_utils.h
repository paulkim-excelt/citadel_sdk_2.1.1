...
typedef q_status_t (*q_yield)(void);
...
q_status_t q_ctx_init (q_lip_ctx_t *ctx,             /* QLIP context pointer */
                       u32_t *ctxDataMemPtr,      /* QLIP data memory pointer */
                       u32_t ctxDataMemSize,      /* QLIP data memory size (in words) */
...
q_status_t q_yield_default (void);                       /* Default yield function */
...
q_status_t q_cfg_counter_spadpa (q_lip_ctx_t *ctx,   /* QLIP context pointer */
                                 u32_t ESCAPE,    /* flags to configure SPA/DPA counter measure settings */
                                 u32_t seed,      /* seed valud to LFSR */
                                 void (*q_get_random)(LFSR_S *lfsr, s32_t length, u32_t *rnd)); /* get_random function pointer */
...
void q_seed_lfsr(LFSR_S *lfsr, u32_t seed);
...
void q_shift_lfsr(LFSR_S *lfsr, s32_t count);
/*!< function to shift LFSR
    \param lfsr pointer to the lfsr that can be used to extract the random data
    \param count number of steps to shift
*/

void q_get_random_default(LFSR_S *lfsr, s32_t length, u32_t *rnd);
