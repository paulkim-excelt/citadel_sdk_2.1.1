...
typedef u32_t            q_limb_t;
typedef q_limb_t*           q_limb_ptr_t;
typedef const q_limb_t*     q_srcptr_t;
typedef u32_t            q_size_t;

#ifndef q_status_t
  typedef s32_t             q_status_t;
#endif
...
typedef q_lint_t*          q_lint_ptr_t;
typedef const q_lint_ptr_t q_lint_srcptr_t;
...
typedef struct q_lip_ctx { 
  ...
  u32_t CurMemLmt;        //!< QLIP context data memory size in words
  u32_t CurMemPtr;        //!< Pointer that points to the next unused QLIP context address
  u32_t *CtxMem;          //!< Pointer to the start of QLIP context data memory
#ifdef QLIP_USE_COUNTER_SPADPA
  u32_t ESCAPE;           //!< Flag to configure SPA/DPA counter measure
  LFSR_S     lfsr;
  void (*q_get_random)(LFSR_S *lfsr, s32_t length, u32_t *rnd);
#endif
...
//__inline u32_t q_asm_addc(u32_t al, u32_t bl, u32_t *carry);
...
//__inline u64_t q_64b_mul(u64_t r, u32_t a, u32_t b);
...
//__inline u64_t q_64b_add(u64_t a, u64_t b);
/*!< 64-bit addition, may be mapped to a single ins.
    \param a source operand
    \param b source operand
*/

q_status_t q_uadd (q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b);
...
q_status_t q_add (q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b);
...
q_status_t q_usub (q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b);
...
q_status_t q_sub (q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b);
...
q_status_t q_ucmp (q_lint_srcptr_t a, q_lint_srcptr_t b);
/*!< unsigned long integer comparison (a>b) return 1; (a<b) return -1; (a==b) return 0;
    \param a source
    \param b source
*/

q_status_t q_cmp (q_lint_srcptr_t a, q_lint_srcptr_t b);
/*!< signed long integer comparison (a>b) return 1; (a<b) return -1; (a==b) return 0;
    \param a source
    \param b source
*/

q_status_t q_modadd (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b, q_lint_srcptr_t n);
...
q_status_t q_modsub (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b, q_lint_srcptr_t n);
...
q_status_t q_mod (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t n);
...
q_status_t q_mod_partial (q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t n);
...
q_status_t q_mod_2pn (q_lint_ptr_t z, q_lint_ptr_t a, u32_t bits);
...
q_status_t q_mul (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b);
...
q_status_t q_sqr (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_ptr_t a);
...
q_status_t q_modmul (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t b, q_lint_srcptr_t n);
...
q_status_t q_modsqr (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_ptr_t a, q_lint_srcptr_t n);
...
q_status_t q_modmul_pb (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_srcptr_t r, q_lint_ptr_t a, q_lint_srcptr_t b, q_lint_srcptr_t n);
...
q_status_t q_modsqr_pb (q_lip_ctx_t *ctx, q_lint_ptr_t z, q_lint_srcptr_t r, q_lint_ptr_t a, q_lint_srcptr_t n);
...
q_status_t q_mul_2pn (q_lint_ptr_t z, q_lint_ptr_t a, u32_t bits);
...
q_status_t q_div_2pn (q_lint_t *z, q_lint_t *a, u32_t bits);
