 ...
 * @brief SPI slave callback function
 */
typedef void (*spi_slave_cb)(void *arg);

/**
 * @typedef spi_api_slave_cb_register
 * @brief API to register callback for slave mode reads
 * See spi_slave_cb_register() for argument descriptions
 */
typedef int (*spi_api_slave_cb_register)(
	struct spi_config *cfg, spi_slave_cb cb, void *arg);
...
struct spi_driver_api {
	...
	spi_api_slave_cb_register register_slave_cb;
};
 ...
 */
static inline int spi_slave_cb_register(
		struct spi_config *config, spi_slave_cb cb, void *arg)
{
	const struct spi_driver_api *api = config->dev->driver_api;

	return api->register_slave_cb(config, cb, arg);
}
