...
#include <crypto/mbedtls/sha256_alt.h>
...
s32_t mbedtls_sha256_starts_ret(mbedtls_sha256_context *ctx, int is224)
#else
s32_t mbedtls_sha256_starts(mbedtls_sha256_context *ctx, int is224)
#endif
{
	if (is224)
...
#ifdef MBEDTLS_VERSION_2_16
s32_t mbedtls_internal_sha256_process(mbedtls_sha256_context *ctx,
			     const u8_t data[BLOCK_SIZE])
#else
s32_t mbedtls_sha256_process(mbedtls_sha256_context *ctx,
			     const u8_t data[BLOCK_SIZE])
#endif
{
	return 0;
...
#ifdef MBEDTLS_VERSION_2_16
void mbedtls_sha256_update(mbedtls_sha256_context *ctx, const u8_t *input,
				size_t ilen)
{
	mbedtls_sha256_update_ret(ctx, input, ilen);
}
s32_t mbedtls_sha256_update_ret(mbedtls_sha256_context *ctx, const u8_t *input,
				size_t ilen)
#else
s32_t mbedtls_sha256_update(mbedtls_sha256_context *ctx, const u8_t *input,
				size_t ilen)
#endif
{
	BCM_SCAPI_STATUS rv;
	...
	if (ctx->last_block_present) {
		if (!ctx->init) {
			rv = crypto_symmetric_hmac_sha_init(
				...
				BCM_SCAPI_AUTH_ALG_SHA256, ctx->last_block,
	...
	if ((ilen % SHA256_BLOCK_SIZE) != 0) {
		if (!ctx->init) {
			...
			rv = crypto_symmetric_hmac_sha256(
			...
			rv = crypto_symmetric_hmac_sha_final(
				(crypto_lib_handle *)handle,
				BCM_SCAPI_AUTH_ALG_SHA256, (u8_t *)input,
...
#ifdef MBEDTLS_VERSION_2_16
void mbedtls_sha256_finish(mbedtls_sha256_context *ctx, u8_t output[32])
{
	mbedtls_sha256_finish_ret(ctx, output);
}
s32_t mbedtls_sha256_finish_ret(mbedtls_sha256_context *ctx, u8_t output[32])
#else
s32_t mbedtls_sha256_finish(mbedtls_sha256_context *ctx, u8_t output[32])
#endif
{
	BCM_SCAPI_STATUS rv;
