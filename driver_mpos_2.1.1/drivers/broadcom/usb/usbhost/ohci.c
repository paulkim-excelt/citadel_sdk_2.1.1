...
void q_enqueue(queue_t *q_1, queue_t *q_2);
queue_t *q_deqnext(queue_t *q);
...
(*((volatile u32_t *) ((softc)->ohci_regs + ((x)/sizeof(u32_t)))) = (y))
#define OHCI_READCSR(softc, x) \
(*((volatile u32_t *) ((softc)->ohci_regs + ((x)/sizeof(u32_t)))))
...
extern const usb_hcdrv_t ohci_driver;
...
void ohci_dumped(ohci_softc_t *softc, ohci_ed_t *ed)
{
	...
	printf("[%p] Ctl=%x (MPS=%d%s%s%s, EN=%d, FA=%d, D=%s) ",
	       OHCI_VTOP(ed),
	       ctl,
	       G_OHCI_ED_MPS(ctl),
	...
	printf("Tailp=%x headp=%x next=%x %s\n",
	       BSWAP32(ed->ed_tailp),
	       BSWAP32(ed->ed_headp),
...
{
	printf("Control: %08X ", reg);
	printf("CBSR=%02x ", G_OHCI_CONTROL_CBSR(reg));
	...
	if (reg & M_OHCI_CONTROL_BLE)
		printf("BLE ");
	printf("HCFS=%02x ", G_OHCI_CONTROL_HCFS(reg));
...
{
	printf("FM Interval: %08X ", reg);
	if (reg & M_OHCI_FMINTERVAL_FIT)
		printf("FIT ");
	printf("FSMPS=%02x ", G_OHCI_FMINTERVAL_FSMPS(reg));
	printf("FI=%02x ", G_OHCI_FMINTERVAL_FI(reg));
	printf("\n");
}

void ohci_dump_fmremaining(u32_t reg)
{
	printf("FM Remaining: %08X ", reg);
	if (reg & M_OHCI_FMREMAINING_FRT)
		printf("FRT ");
	printf("FR=%02x ", G_OHCI_FMREMAINING_FR(reg));
	printf("\n");
}
...
{
	printf("HCCA:\n");
	ohci_dump_control(OHCI_READCSR(softc, R_OHCI_CONTROL));
	ohci_dump_status(OHCI_READCSR(softc, R_OHCI_CMDSTATUS));
	ohci_dump_int("Int Status ", OHCI_READCSR(softc, R_OHCI_INTSTATUS));
	ohci_dump_int("Int Enable ", OHCI_READCSR(softc, R_OHCI_INTENABLE));
	ohci_dump_int("Int Disable", OHCI_READCSR(softc, R_OHCI_INTDISABLE));
	printf("hcca:			   %08x\n",
	       OHCI_READCSR(softc, R_OHCI_HCCA));
	printf("Period CurrentED:  %08x\n",
	       OHCI_READCSR(softc, R_OHCI_PERIODCURRENTED));
	printf("Control HeadED:	%08x\n",
	       OHCI_READCSR(softc, R_OHCI_CONTROLHEADED));
	printf("Control CurrentED: %08x\n",
	       OHCI_READCSR(softc, R_OHCI_CONTROLCURRENTED));
	printf("Bulk HeadED:	   %08x\n",
	       OHCI_READCSR(softc, R_OHCI_BULKHEADED));
	printf("Bulk CurrentED:	%08x\n",
	       OHCI_READCSR(softc, R_OHCI_BULKCURRENTED));
	printf("Done Head:		   %08x\n",
	       OHCI_READCSR(softc, R_OHCI_DONEHEAD));
	ohci_dump_fminterval(OHCI_READCSR(softc, R_OHCI_FMINTERVAL));
	ohci_dump_fmremaining(OHCI_READCSR(softc, R_OHCI_FMREMAINING));
	printf("FM Number:		   %08x\n",
	       OHCI_READCSR(softc, R_OHCI_FMNUMBER));
	printf("Periodic Start:	%08x\n",
	       OHCI_READCSR(softc, R_OHCI_PERIODICSTART));
	printf("LSThreshold:	   %08x\n",
	       OHCI_READCSR(softc, R_OHCI_LSTHRESHOLD));
	ohci_dumprhstat(OHCI_READCSR(softc, R_OHCI_RHSTATUS));
	ohci_dumpportstat(1, OHCI_READCSR(softc, R_OHCI_RHPORTSTATUS(1)));
	ohci_dumpportstat(2, OHCI_READCSR(softc, R_OHCI_RHPORTSTATUS(2)));
...
static ohci_transfer_t *_ohci_allocxfer(ohci_softc_t *softc)
{
	...
	if(t->t_next == NULL) {	printf (" ====>A_ALERT \n");}
...
static void _ohci_freexfer(ohci_softc_t *softc, ohci_transfer_t *t)
{
	...
	if(t->t_next == NULL) {printf (" ====>F_ALERT t_next is NULL \n");}
}
...
static int _ohci_initpools(ohci_softc_t *softc)
{
	...
	if (!softc->ohci_hcca) {
		printf("Could not allocate host communication area\n");
		return -1;
	}
...
static int ohci_start(usbbus_t *bus)
{
	...
	/* Enable interrupts */
	OHCI_WRITECSR(softc, R_OHCI_INTENABLE, M_OHCI_INT_ALL);
	OHCI_WRITECSR(softc, R_OHCI_INTDISABLE, M_OHCI_INT_SF);
	OHCI_WRITECSR(softc, R_OHCI_INTDISABLE, M_OHCI_INT_FNO);
#endif
...
static void ohci_intr_procdoneq(ohci_softc_t *softc)
{
	...
	while (doneq) {
		...
		invalidate_dcache_by_addr((u32_t)ur, sizeof(usbreq_t));
#endif
...
void ohci_roothub_reset(ohci_softc_t *softc, int statport)
{
	...
	for (i = 0; i < 5; i++) {	/* FIXME: timer */
		usb_delay_ms(100);
		if (!(OHCI_READCSR(softc, statport) & M_OHCI_RHPORTSTAT_PRS))
			break;
	}
}
...
static int ohci_intr(usbbus_t *bus)
{
...
#ifdef USBHOST_INTERRUPT_MODE
	enabled = OHCI_READCSR(softc, R_OHCI_INTENABLE);
	...
	if (reg & M_OHCI_INT_SO) {
		reg &= ~M_OHCI_INT_SO;
		printf("SchedOverrun\n");
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_SO);
	}
	...
	if (reg & M_OHCI_INT_WDH) {
		...
		/* printf("DoneQueue\n"); */
		ohci_intr_procdoneq(softc);
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_WDH);
	}
	...
	if (reg & M_OHCI_INT_SF) {
		...
		reg &= ~M_OHCI_INT_SF;
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_SF);
	}
	...
	if (reg & M_OHCI_INT_RD) {
		reg &= ~M_OHCI_INT_RD;
		printf("ResumeDetect\n");
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_RD);
	}
	...
	if (reg & M_OHCI_INT_UE) {
		...
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_UE);
	}
	...
	if (reg & M_OHCI_INT_FNO) {
		...
		reg &= ~M_OHCI_INT_FNO;
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_FNO);
	}

	/* Root Hub Status Change */
	if ((reg) & M_OHCI_INT_RHSC) {
		reg &= ~M_OHCI_INT_RHSC;
		printf("roothub_statchg\n");
		ohci_roothub_statchg(softc);
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_RHSC);
	}
	...
	if (reg & M_OHCI_INT_OC) {
		...
		OHCI_WRITECSR(softc, R_OHCI_INTSTATUS, M_OHCI_INT_OC);
	}

	if (reg) {
		printf("ohci reg=%08x\n", reg);
		if (reg & M_OHCI_INT_RHSC) {
			printf("softc->ohci_intdisable=%d\n",
...

	{
		...
		if (ohci_reg->rhport00 & M_OHCI_RHPORTSTAT_PES) {
			if (flag) {
				flag = 0;
				printf("OHCI port enable rhport00=%x\n",
				       ohci_reg->rhport00);
			}
		} else {
			flag = 1;
	...
	}
	{
		...
		if (ohci_reg->rhport00 & M_OHCI_RHPORTSTAT_PSS) {
			if (flag) {
				flag = 0;
				printf("OHCI port suspend rhport00=%x\n",
				       ohci_reg->rhport00);
			}
		} else {
			flag = 1;
	...
	}
	{
		...
		if (ohci_reg->rhport00 & M_OHCI_RHPORTSTAT_PRS) {
			if (flag) {
				flag = 0;
				printf("OHCI port reset rhport00=%x\n",
				       ohci_reg->rhport00);
			}
		} else {
			flag = 1;
	...
	}
	{
		...
		if (ohci_reg->rhport00 & M_OHCI_RHPORTSTAT_CSC) {
			if (flag) {
				flag = 0;
				printf("OHCI port connect change rhport00=%x\n",
				       ohci_reg->rhport00);
	...
	}
	{
		...
		if (ohci_reg->rhport00 & M_OHCI_RHPORTSTAT_PESC) {
			if (flag) {
				flag = 0;
				printf("OHCI port enable change rhport00=%x\n",
				       ohci_reg->rhport00);
			}
		} else {
			flag = 1;
	...
	}
	{
		...
		if (ohci_reg->rhport00 & M_OHCI_RHPORTSTAT_PSSC) {
			if (flag) {
				flag = 0;
				printf("OHCI port suspend change rhport00=%x\n",
				       ohci_reg->rhport00);
			}
		} else {
			flag = 1;
	...
	}
	{
		...
		if (ohci_reg->rhport00 & M_OHCI_RHPORTSTAT_PRSC) {
			if (flag) {
				flag = 0;
				printf("OHCI port reset change rhport00=%x\n",
				       ohci_reg->rhport00);
			}
		} else {
			flag = 1;
 ...
 */
static usbbus_t usb_bus;
static ohci_softc_t ohci_soft;

static usbbus_t *ohci_create(u32_t addr)
{
	ohci_softc_t *softc;
	usbbus_t *bus;
	...
	bus->ub_hwdisp = (usb_hcdrv_t *)&ohci_driver;
	...
	if ((_ohci_initpools(softc)) != 0)
		goto error;
	if ((_ohci_setupepts(softc)) != 0)
		goto error;
				  ...
				  int usbaddr, int eptnum, int mps, int flags)
{
...
#if 0
	/* set different TD type for different EP flag. */
	td = (ohci_td_t *) tailtd;
#endif
#if 0
	usbEndpoint = (u32_t) ept;
	...
	if (flags & UP_TYPE_CONTROL) {
		...
		_ohci_queueept(softc, softc->ohci_isoc_list, ept);
#endif
		printf("WARNING: ISOC not supported\n");
		return NULL;
	}
...
static int ohci_cancel_xfer(usbbus_t *bus, usb_ept_t *uept, usbreq_t *ur)
{
...
static int ohci_iso_xfer(usbbus_t *bus, usb_ept_t *uept, usbreq_t *ur)
{
	printf("WARNING: ISOC not supported \n");
	ARG_UNUSED(bus);
	ARG_UNUSED(uept);
	...
	startFrame = OHCI_READCSR(softc, R_OHCI_FMNUMBER) + 10;
...
static int ohci_xfer(usbbus_t *bus, usb_ept_t *uept, usbreq_t *ur)
{
	...
	if (len == 0) {
		...
		if(newtailtransfer == NULL) {
			...
			printf("\n ======> WARNING no free xfer: %d  %p======> \n",__LINE__,softc->ohci_transfer_freelist);
		}
		...
		while (len > 0) {
			...
			if(softc->ohci_transfer_freelist == NULL) {
				//something is seriouly wrong
				printf("\n ======> WARNING no free xfer left: %d %p len:%d======>",__LINE__,softc->ohci_transfer_freelist,len);
			}
 ...
 */
static const usb_device_descr_t ohci_root_devdsc = {
...
static const usb_config_descr_t ohci_root_cfgdsc = {
...
static const usb_interface_descr_t ohci_root_ifdsc = {
...
static const usb_endpoint_descr_t ohci_root_epdsc = {
...
static const usb_hub_descr_t ohci_root_hubdsc = {
...
static int ohci_roothub_req(ohci_softc_t *softc, usb_device_request_t *req)
{
	...
	case REQCODE(USB_REQUEST_GET_STATUS, USBREQ_DIR_IN, USBREQ_TYPE_CLASS,
		     USBREQ_REC_OTHER):
		status =
		    OHCI_READCSR(softc, (R_OHCI_RHPORTSTATUS(wIndex)));
#ifdef _OHCI_DEBUG_
		if (ohcidebug > 0) {
			printf("RHGetStatus: ");
...
#ifdef _OHCI_DEBUG_
		if (ohcidebug > 0) {
			printf("RHClearFeature(%d): ", wValue);
...
#ifdef USBHOST_INTERRUPT_MODE
				OHCI_WRITECSR(softc, R_OHCI_INTENABLE,
					      M_OHCI_INT_RHSC);
#endif
			}
		}
...
static void ohci_roothub_statchg(ohci_softc_t *softc)
{
	...
	if (portstat != 0) {
#ifdef USBHOST_INTERRUPT_MODE
		OHCI_WRITECSR(softc, R_OHCI_INTDISABLE, M_OHCI_INT_RHSC);
#endif
		softc->ohci_intdisable |= M_OHCI_INT_RHSC;
...
static int ohci_roothub_xfer(usbbus_t *bus, usb_ept_t *uept, usbreq_t *ur)
{
	...
	case 0:
		...
		if (ur->ur_flags & UR_FLAG_SETUP) {
			...
			if (res)
				printf("Root hub request returned an error\n");
...
{
	printf("==================OHCI REG DUMP====================\n");
	printf("revision		 (%p)=%08x\n", &ohci_reg->revision,
	       ohci_reg->revision);
	printf("control		 (%p)=%08x\n", &ohci_reg->control,
	       ohci_reg->control);
	printf("cmdstatus		 (%p)=%08x\n", &ohci_reg->cmdstatus,
	       ohci_reg->cmdstatus);
	printf("intstatus		 (%p)=%08x\n", &ohci_reg->intstatus,
	       ohci_reg->intstatus);
	printf("intenable		 (%p)=%08x\n", &ohci_reg->intenable,
	       ohci_reg->intenable);
	printf("intdisable		 (%p)=%08x\n", &ohci_reg->intdisable,
	       ohci_reg->intdisable);
	printf("hcca			 (%p)=%08x\n", &ohci_reg->hcca,
	       ohci_reg->hcca);
	printf("periodcurrented (%p)=%08x\n", &ohci_reg->periodcurrented,
	       ohci_reg->periodcurrented);
	printf("controlheaded	 (%p)=%08x\n", &ohci_reg->controlheaded,
	       ohci_reg->controlheaded);
	printf("controlcurrented(%p)=%08x\n", &ohci_reg->controlcurrented,
	       ohci_reg->controlcurrented);
	printf("bulkheaded		 (%p)=%08x\n", &ohci_reg->bulkheaded,
	       ohci_reg->bulkheaded);
	printf("bulkcurrented	 (%p)=%08x\n", &ohci_reg->bulkcurrented,
	       ohci_reg->bulkcurrented);
	printf("donehead		 (%p)=%08x\n", &ohci_reg->donehead,
	       ohci_reg->donehead);
	printf("fminterval		 (%p)=%08x\n", &ohci_reg->fminterval,
	       ohci_reg->fminterval);
	printf("fmremaining	 (%p)=%08x\n", &ohci_reg->fmremaining,
	       ohci_reg->fmremaining);
	printf("fmnumber		 (%p)=%08x\n", &ohci_reg->fmnumber,
	       ohci_reg->fmnumber);
	printf("periodicstart	 (%p)=%08x\n", &ohci_reg->periodicstart,
	       ohci_reg->periodicstart);
	printf("lsthreshold	 (%p)=%08x\n", &ohci_reg->lsthreshold,
	       ohci_reg->lsthreshold);
	printf("rhdscra		 (%p)=%08x\n", &ohci_reg->rhdscra,
	       ohci_reg->rhdscra);
	printf("rhdscrb		 (%p)=%08x\n", &ohci_reg->rhdscrb,
	       ohci_reg->rhdscrb);
	printf("rhstatus		 (%p)=%08x\n", &ohci_reg->rhstatus,
	       ohci_reg->rhstatus);
	printf("rhport00		 (%p)=%08x\n", &ohci_reg->rhport00,
	       ohci_reg->rhport00);
	printf("rhport01		 (%p)=%08x\n", &ohci_reg->rhport01,
	       ohci_reg->rhport01);
	printf("rhport02		 (%p)=%08x\n", &ohci_reg->rhport02,
	       ohci_reg->rhport02);
	printf("phyctrlp0		 (%p)=%08x\n", &ohci_reg->phyctrlp0,
	       ohci_reg->phyctrlp0);
	printf("framelenadj	 (%p)=%08x\n", &ohci_reg->framelenadj,
	       ohci_reg->framelenadj);
	printf("shimctrl		 (%p)=%08x\n", &ohci_reg->shimctrl,
	       ohci_reg->shimctrl);
	printf("shimslavestatus (%p)=%08x\n", &ohci_reg->shimslavestatus,
	       ohci_reg->shimslavestatus);
	printf("shimmasterstatus(%p)=%08x\n", &ohci_reg->shimmasterstatus,
	       ohci_reg->shimmasterstatus);
	printf("ehcistatus		 (%p)=%08x\n", &ohci_reg->ehcistatus,
	       ohci_reg->ehcistatus);
	printf("utmip0ctl		 (%p)=%08x\n", &ohci_reg->utmip0ctl,
	       ohci_reg->utmip0ctl);
	printf("tpctl			 (%p)=%08x\n", &ohci_reg->tpctl,
	       ohci_reg->tpctl);
	printf("tpoutp0		 (%p)=%08x\n", &ohci_reg->tpoutp0,
	       ohci_reg->tpoutp0);
	printf("ohcipwrsts		 (%p)=%08x\n", &ohci_reg->ohcipwrsts,
	       ohci_reg->ohcipwrsts);
	printf("ehcixfer		 (%p)=%08x\n", &ohci_reg->ehcixfer,
	       ohci_reg->ehcixfer);
	printf("ehcilpsmcstate  (%p)=%08x\n", &ohci_reg->ehcilpsmcstate,
	       ohci_reg->ehcilpsmcstate);
	printf("ohciehcistrap	 (%p)=%08x\n", &ohci_reg->ohciehcistrap,
	       ohci_reg->ohciehcistrap);
	printf("ohciccssts		 (%p)=%08x\n", &ohci_reg->ohciccssts,
	       ohci_reg->ohciccssts);
	printf("intrcontrol	 (%p)=%08x\n", &ohci_reg->intrcontrol,
	       ohci_reg->intrcontrol);
	printf("ehciincrxstrap  (%p)=%08x\n", &ohci_reg->ehciincrxstrap,
	       ohci_reg->ehciincrxstrap);
	printf("===================================================\n");
}
