...
#define CV_RETAIN_VOLATILE_OBJS		0x00000040
#define CV_OPTIONS_MASK (CV_SECURE_SESSION | CV_USE_SUITE_B | CV_SYNCHRONOUS | CV_SUPPRESS_UI_PROMPTS \
| CV_INTERMEDIATE_CALLBACKS | CV_FP_INTERMEDIATE_CALLBACKS | CV_RETAIN_VOLATILE_OBJS)
...
#define CV_FINGER_WAS_ALREADY_PLACED_USE_SESSION_ID 0x00000200
/* this mask defines the only bits valid for input parameter */
#define CV_FP_CONTROL_MASK (CV_INVALIDATE_CAPTURE | CV_HINT_AVAILABLE | CV_USE_FAR | CV_USE_FEATURE_SET | CV_USE_FEATURE_SET_HANDLE | CV_USE_PERSISTED_IMAGE_ONLY | CV_CONTAINS_CAPTURE_ID | CV_ASYNC_FP_CAPTURE_ONLY | CV_FINGER_WAS_ALREADY_PLACED_USE_SESSION_ID)
...
#define CV_ATTRIB_NOT_IN_GLB_BKUP		0x00400000

#define CV_MAJOR_VERSION_1 1
#define CV_MINOR_VERSION_0 0

#define CV_MAJOR_VERSION 2
#define CV_MINOR_VERSION 0

#define CV_CAPABILITY_POA_BASIC 0x00000001
#define CV_CAPABILITY_POA_OPERATIONAL 0x00000002
...
#define DH1024_KEY_LEN 128
#define DH1024_KEY_LEN_BITS 1024
#define DH2048_KEY_LEN 256
#define DH2048_KEY_LEN_BITS 2048
...
#define ECC512_KEY_LEN 64
#define ECC512_KEY_LEN_BITS ECC512_KEY_LEN*8
#define ECC512_SIG_LEN 2*ECC512_KEY_LEN
#define ECC512_POINT (2*ECC5126_KEY_LEN)
#define ECC512_SIZE_R	ECC512_KEY_LEN
#define ECC512_SIZE_S	ECC512_KEY_LEN
#define WBF_CONTROL_UNIT_SESSION_ID_SIZE 16
#define MAX_CV_COMMAND_LENGTH 4096
#define MAX_CV_OBJ_SIZE (2048*7) /* maximum object size in bytes */

#define CV_STACK_BUFFER_SIZE (MAX_CV_OBJ_SIZE + SHA256_LEN + AES_256_BLOCK_LEN) 
...
#define CV_GCK_KEY_HANDLE_LEN 32
...
enum cv_status_e {
		...
		CV_CONTACTLESS_COM_ERR,				// 0x0000009b
		CV_DO_REENUM,						// 0x0000009c
		CV_NFC_NOT_PRESENT,					// 0x0000009d
		CV_HOTP_SECOND_OTP_REQUIRED,		// 0x0000009e		
        CV_FP_CAPTURE_FAILED_MODERN_STANDBY,// 0x0000009f
...
/* fingerprint prompt related return codes */
		...
		CV_PROMPT_FOR_FIFTH_FINGERPRINT_TOUCH,
		CV_PROMPT_FOR_SIXTH_FINGERPRINT_TOUCH,
		CV_PROMPT_FOR_SEVENTH_FINGERPRINT_TOUCH,
		CV_PROMPT_FOR_EIGHTH_FINGERPRINT_TOUCH,
		CV_PROMPT_FOR_NINETH_FINGERPRINT_TOUCH,
		CV_PROMPT_FOR_TENTH_FINGERPRINT_TOUCH,
...
enum cv_status_type_e {
		...
		CV_STATUS_POA_TEST_RESULT,
		CV_STATUS_IS_POA_ENABLED,
		CV_STATUS_CAPABILITIES
};
...
enum cv_fingerprint_type_e {
		...
		CV_FINGERPRINT_FEATURE_SET_SYNAPTICS,
		CV_FINGERPRINT_TEMPLATE_SYNAPTICS_WITHOUT_ANTISPOOFING,
		CV_FINGERPRINT_TEMPLATE_SYNAPTICS
};
...
typedef unsigned int cv_cl_event;
#define CV_CL_EVENT_PRESENCE		1

typedef unsigned int cv_cl_event_type;
#define CV_CL_EVENT_ARRIVAL			1
#define CV_CL_EVENT_REMOVAL			2
...
typedef unsigned int cv_system_power_state;
enum cv_system_power_state_e {
	CV_SYSTEM_STANDBY,
	CV_SYSTEM_HIBERNATE
};
...
#define CV_NFC_ENABLED   0x1000
#define CV_NFC_PRESENT   0x2000
#define CV_ROUTE_MIFARE_DESFIRE_14443A_TO_RFID	0x4000
#define CV_ROUTE_ISO14443A_TO_RFID   0x8000
#define CV_ROUTE_ISO14443B_TO_RFID   0x10000
#define CV_ROUTE_MIFARE_DESFIRE_14443A_TO_NFC   0x20000
#define CV_ROUTE_ISO14443A_TO_NFC   0x40000
#define CV_ROUTE_ISO14443B_TO_NFC   0x80000
#define CV_ROUTE_T3T_TO_NFC   0x100000
#define CV_ROUTE_T3T_TO_RFID   0x200000
#define CV_EC_IS_PRESENT   	  0x400000
#define CV_EC_IS_NOT_PRESENT   0x800000

#define CV_FP_NEXT_SENSOR_DETECTED   0x1000000
#define CV_FP_DP_SENSOR_DETECTED   0x2000000
#define CV_FP_SYNAPTICS_SENSOR_DETECTED   0x4000000


#define CV_ROUTE_MASK (CV_ROUTE_MIFARE_DESFIRE_14443A_TO_RFID | CV_ROUTE_ISO14443A_TO_RFID | CV_ROUTE_ISO14443B_TO_RFID | \
						CV_ROUTE_MIFARE_DESFIRE_14443A_TO_NFC |  CV_ROUTE_ISO14443A_TO_NFC | CV_ROUTE_ISO14443B_TO_NFC  | \
						CV_ROUTE_T3T_TO_NFC | CV_ROUTE_T3T_TO_RFID )


typedef unsigned int cv_logging_enables;
#define CV_HID_LOGGING_ENABLED 0x1
#define CV_NFC_LOGGING_ENABLED 0x2
...
typedef unsigned int cv_function_enables;
enum cv_function_enables_e {
	CV_ENABLE_CLSC_NOTIFICATION,
	CV_DISABLE_CLSC_NOTIFICATION
};
...
#define CV_PURPOSE_MASK (CV_FE_CONTROL_PURPOSE_ENROLLMENT | CV_FE_CONTROL_PURPOSE_VERIFY | CV_FE_CONTROL_PURPOSE_IDENTIFY)

/*
 * Structures
 */

typedef struct td_cv_auth_list_hdr {
...
} PACKED cv_auth_list_hdr;
typedef cv_auth_list_hdr cv_auth_list;

typedef struct td_cv_attrib_hdr {
	cv_obj_attribute_type	attribType;		/* type of attribute */
	uint16_t				attribLen;		/* length of attribute in bytes */
} PACKED  cv_attrib_hdr;
typedef cv_attrib_hdr cv_obj_attributes;

typedef struct td_cv_auth_hdr {
	cv_object_authorizations	authType;		/* type of authorization */
	uint16_t					authLen;		/* length of authorization in bytes */
} PACKED  cv_auth_hdr;

typedef struct td_cv_obj_hdr {
	cv_obj_type			objType;	/* type of object */
	uint16_t			objLen;		/* length of object in bytes, excluding objType and objLen */
} PACKED  cv_obj_hdr;

typedef struct td_cv_auth_entry_passphrase {
	uint8_t passphraseLen;		/* length of passphrase */
	uint8_t passphrase[1];		/* passphrase byte string */
} PACKED  cv_auth_entry_passphrase;

typedef struct td_cv_auth_entry_passphrase_handle {
	uint8_t			constZero;		/* value: 0 */
	cv_obj_handle	hPassphrase;	/* handle to passphrase object */
} PACKED  cv_auth_entry_passphrase_handle;

typedef struct td_cv_auth_entry_passphrase_HMAC {
	uint8_t HMACLen;	/* length of HMAC; 20 bytes for SHA1 and 32 bytes for SHA-256 */
	uint8_t HMAC[1];	/* HMAC */
} PACKED  cv_auth_entry_passphrase_HMAC;

typedef struct td_cv_auth_entry_fingerprint_obj {
	uint8_t			numTemplates;	/* number of templates */
	cv_obj_handle	templates[1];	/* list of template handles */
} PACKED  cv_auth_entry_fingerprint_obj;

typedef struct td_cv_auth_entry_hotp {
	cv_obj_handle	hHOTPObj;	/* handle for HOTP object */
} PACKED  cv_auth_entry_hotp;

typedef struct td_cv_auth_hotp_command {
	uint64_t	OTPValue; /* 6, 8 or 10 digit OTP value for authentication */
} PACKED  cv_auth_hotp_command;

typedef struct td_cv_auth_entry_auth_session {
	cv_obj_handle	hAuthSessionObj;	/* handle for auth session object */
} PACKED  cv_auth_entry_auth_session;

typedef struct td_cv_auth_entry_st_response_obj {
	uint32_t		crc32[256];		/* crc table */
} PACKED  cv_auth_entry_st_response_obj;

typedef struct td_cv_auth_entry_st_response {
	uint8_t			respHash[SHA1_LEN];	/* response hash */
} PACKED  cv_auth_entry_st_response;

typedef struct td_cv_auth_data {
	uint8_t	authData[20];	/* authorization data */
} PACKED  cv_auth_data;

typedef struct td_cv_nonce {
	uint8_t	nonce[20];	/* nonce */
} PACKED  cv_nonce;

typedef struct td_cv_hotp_unblock_auth {
	uint64_t	OTPValue1;
	uint64_t	OTPValue2;
} PACKED  cv_hotp_unblock_auth;

typedef struct td_cv_otp_nonce {
	uint8_t	nonce[AES_128_BLOCK_LEN];	/* nonce */
} PACKED  cv_otp_nonce;

typedef struct td_cv_wbf_getsessionid
{
	uint8_t sessionID[WBF_CONTROL_UNIT_SESSION_ID_SIZE];
	cv_obj_handle template_id_of_identified_fingerprint;
} PACKED  cv_wbf_getsessionid;

typedef struct td_cv_auth_fp_with_session_id {
	uint8_t sessionID[WBF_CONTROL_UNIT_SESSION_ID_SIZE];
} PACKED  cv_auth_fp_with_session_id;

typedef struct td_cv_time {
	uint8_t	time[7];	/* time in hex: YYYYMMDDHHMMSS (eg. 0x20,0x05,0x12,0x14,0x21,0x10,0x00 for  2005 Dec 14 21:10:00) */
} PACKED  cv_time;

typedef struct td_cv_auth_entry_count {
	uint32_t	count;	/* count of times object usage allowed */
} PACKED  cv_auth_entry_count;

typedef struct td_cv_auth_entry_time {
	cv_time	startTime;	/* start time for authorization */
	cv_time	endTime;	/* end time for authorization */
} PACKED  cv_auth_entry_time;

typedef struct td_cv_auth_entry_contactless {
...
} PACKED  cv_auth_entry_contactless;

typedef struct td_cv_auth_entry_contactless_handle {
	cv_contactless_type	type;	/* contactless credential type */
	uint16_t	constZero;		/* value: 0 */
	cv_obj_handle	hHIDObj;	/* handle to object containing HID credential */
} PACKED  cv_auth_entry_contactless_handle;

typedef struct td_cv_auth_entry_smartcard_handle {
	uint16_t	constZero;		/* value: 0 */
	cv_obj_handle	keyObj;		/* handle to key object */
} PACKED  cv_auth_entry_smartcard_handle;

typedef struct td_cv_auth_entry_challenge {
	cv_nonce	nonce;			/* nonce */
	uint8_t		*signature;		/* signature */
} PACKED  cv_auth_entry_challenge;

typedef struct td_cv_auth_entry_challenge_handle_obj {
	uint16_t	constZero;		/* value: 0 */
	cv_obj_handle	keyObj;		/* handle to key object */
} PACKED  cv_auth_entry_challenge_handle_obj;

typedef struct td_cv_auth_entry_PIN {
	uint8_t PINLen;	/* length of PIN */
	uint8_t PIN[1];	/* PIN byte string */
} PACKED  cv_auth_entry_PIN;

typedef struct td_cv_obj_value_certificate {
	uint16_t	certLen;	/* length of certificate */
	uint8_t	cert[1];		/* certificate */
} PACKED  cv_obj_value_certificate;

typedef struct td_cv_obj_value_fingerprint {
	cv_fingerprint_type	type;	/* cv_fingerprint_type */
	uint16_t			fpLen;	/* fingerprint length */
	uint8_t				fp[1];	/* fingerprint */
} PACKED  cv_obj_value_fingerprint;

typedef struct td_cv_obj_value_opaque {
	uint16_t	valueLen;	/* value length in bytes*/
	uint8_t	value[1];		/* value */
} PACKED  cv_obj_value_opaque;

typedef struct td_cv_obj_value_passphrase {
	uint16_t	passphraseLen;	/* value length in bytes*/
	uint8_t	passphrase[1];		/* value */
} PACKED  cv_obj_value_passphrase;

typedef struct td_cv_obj_value_hash {
	cv_hash_type	hashType;	/* cv_hash_type */
	uint8_t			hash[1];	/* hash */
} PACKED  cv_obj_value_hash;

typedef struct td_cv_obj_value_hotp {
...
} PACKED  cv_obj_value_hotp;

typedef struct td_cv_obj_value_aes_key {
	uint16_t	keyLen;	/* key size in bits */
	uint8_t	key[1];		/* key */
} PACKED  cv_obj_value_aes_key;

typedef struct td_cv_obj_value_key {
	uint16_t	keyLen;	/* key size in bits */
	uint8_t	key[1];		/* key */
} PACKED  cv_obj_value_key;

typedef struct td_cv_obj_value_oath_token {
...
} PACKED  cv_obj_value_oath_token;

typedef struct td_cv_obj_value_auth_session {
...
} PACKED  cv_obj_value_auth_session;

typedef struct td_cv_auth_entry_auth_session_HMAC {
	cv_obj_handle	hAuthSessionObj;	/* handle for auth session object */
	cv_obj_value_hash hmac;
} PACKED  cv_auth_entry_auth_session_HMAC;

typedef struct td_cv_auth_entry_smartcard {
	cv_key_type	type;			/* key type */
	cv_obj_value_key key;		/* key */
} PACKED  cv_auth_entry_smartcard;

typedef struct td_cv_auth_entry_challenge_obj {
	cv_key_type	type;			/* key type */
	cv_obj_value_key key;		/* key */
} PACKED  cv_auth_entry_challenge_obj;

typedef struct td_cv_auth_entry_proprietary_sc {
	cv_obj_value_hash	hash;	/* SHA1(user key || public data field) */
} PACKED  cv_auth_entry_proprietary_sc;

typedef struct td_cv_auth_entry_passphrase_HMAC_obj {
	cv_obj_value_hash	hash;
} PACKED  cv_auth_entry_passphrase_HMAC_obj;

typedef struct td_cv_date {
	uint8_t		month;
	uint8_t		day;
	uint16_t	year;
} PACKED  cv_date;

/* structure of object auth entries of type CV_AUTH_CAC_PIV */
typedef struct td_cv_auth_entry_cac_piv_obj {
	cv_cac_piv_card_type	cardType;					/* type of card: CAC or PIV */
	cv_cac_piv_card_signature_alg cardSignAlg;			/* card signature alg: RSA 1024, RSA 2048, ECC 256 */
	cv_date					expirationDate;				/* expiration date of card */
	uint32_t				pubkeyExp;					/* pubkey exponent (RSA only - 0 for ECC) */
	uint8_t                 pubkey[1];					/* pubkey for signature verification: 128, 256 or 64 bytes */
	                                                    /*  for RSA 1024, RSA 2048 and ECC 256, resp. */
} PACKED  cv_auth_entry_cac_piv_obj;

/* structure of object auth entries of type CV_AUTH_PIV_CTLS */
typedef struct td_cv_auth_entry_piv_ctls_obj {
	uint8_t					hCHUID[SHA256_LEN];			/* SHA256 hash of CHUID */
	uint8_t					salt[8];					/* salt for PBKDF2 algorithm used for pseudo-PIN */
	cv_date					expirationDate;				/* expiration date of card */
} PACKED  cv_auth_entry_piv_ctls_obj;

/* structure of parameter list auth entries of type CV_AUTH_CAC_PIV or CV_AUTH_PIV_CTLS */
typedef struct td_cv_auth_entry_cac_piv {
	cv_date					currentDate;				/* current date to check against expiration date */
} PACKED  cv_auth_entry_cac_piv;

typedef struct td_cv_app_id {
	uint8_t	appIDLen;	/* length of application ID */
	uint8_t	appID[1];	/* application ID byte string */
} PACKED  cv_app_id;

typedef struct td_cv_user_id {
	uint8_t	userIDLen;	/* length of application ID */
	uint8_t	userID[1];	/* application ID byte string */
} PACKED  cv_user_id;

typedef struct td_cv_version {
	uint16_t	versionMajor;	/* major version of CV */
	uint16_t	versionMinor;	/* minor version of CV */
} PACKED  cv_version;

typedef struct td_cv_bulk_params {
...
} PACKED  cv_bulk_params;

typedef struct tdotp_rsa_alg_params {
	uint8_t serialNumber[4];
	uint8_t displayDigits;
	uint8_t displayIntervalSeconds;
} PACKED  tpm_otp_rsa_alg_params;

typedef struct td_cv_obj_value_rsa_token {
	cv_otp_token_type type;				/* token type (RSA or OATH) */
	tpm_otp_rsa_alg_params rsaParams;	/* RSA parameters */
	uint8_t clientNonce[AES_128_BLOCK_LEN];	/* client nonce */
	uint8_t sharedSecretLen;			/* length of shared secret */
	uint8_t	sharedSecret[AES_128_BLOCK_LEN];			/* sharedSecret */
} PACKED  cv_obj_value_rsa_token;

typedef struct td_cv_otp_rsa_keygen_params_direct {
...
} PACKED  cv_otp_rsa_keygen_params_direct;

typedef struct td_cv_otp_rsa_keygen_params_ctkip {
	cv_otp_provisioning_type	otpProvType;	/* provisioning type, direct or CT-KIP */
	tpm_otp_rsa_alg_params		rsaParams;		/* RSA params */
	uint8_t						clientNonce[AES_128_BLOCK_LEN];	/* client nonce */
...
} PACKED  cv_otp_rsa_keygen_params_ctkip;

typedef struct td_cv_otp_oath_keygen_params {
	uint8_t						clientNonce[AES_128_BLOCK_LEN];	/* client nonce */
	uint8_t						serverNonce[AES_128_BLOCK_LEN];	/* client nonce */
	uint32_t					serverPubkeyLen; /* length of server public key */
	uint8_t						serverPubkey[1]; /* server public key */
} PACKED  cv_otp_oath_keygen_params;

typedef struct td_cv_space {
...
typedef struct td_cv_da_params {
...
typedef struct td_cv_time_params {
	cv_time	localTime;
	cv_time	GMTTime;
} PACKED  cv_time_params;

typedef struct td_cv_st_challenge {
	uint32_t rowNumber;
	uint8_t nonce[CV_NONCE_LEN];
} PACKED  cv_st_challenge;

typedef struct td_cv_st_hash_buff {
	uint32_t crc32;
	uint8_t nonce[CV_NONCE_LEN];
} PACKED  cv_st_hash_buff;
...
} PACKED  cv_contactless_identifier;

typedef struct td_cv_super_config {
	uint8_t	X[AES_BLOCK_LENGTH];        // challenge encryption key
	uint8_t	R[AES_BLOCK_LENGTH];        // recovery AES key
	uint8_t	Z[AES_BLOCK_LENGTH];        // recovery secret
} PACKED  cv_super_config;

typedef struct td_cv_super_registration_params {
	uint8_t	encZ[AES_BLOCK_LENGTH];		// encrypted recovery secret
	uint8_t	X[AES_BLOCK_LENGTH];        // recovery AES key
	uint8_t	RSN[CV_SUPER_RSN_LEN];		// recovery serial number
} PACKED  cv_super_registration_params;

typedef struct td_cv_super_challenge {
	uint8_t		RSN[CV_SUPER_RSN_LEN];	// recovery serial number
	uint32_t	ctr;        			// counter
	uint8_t		encCtr[4];        		// encrypted counter
} PACKED  cv_super_challenge;
...
cv_status
cv_enumerate_objects_direct (uint32_t userAppHashListLength, uint8_t *pUserAppHashListBuf,
					 cv_obj_type objType, uint32_t *pObjHandleListLength, cv_obj_handle *pObjHandleList);
...
cv_fingerprint_enroll_dup_check (cv_handle cvHandle,
			uint32_t objAttributesLength, cv_obj_attributes *pObjAttributes,
			uint32_t authListsLength, cv_auth_list *pAuthLists,
			uint32_t hashListLength, uint8_t *pHashListBuf,
			uint32_t *pTemplateLength, uint8_t *pTemplate,
			cv_obj_handle *pTemplateHandle,
			cv_callback *callback, cv_callback_ctx context);
...
cv_status
cv_fingerprint_create_feature_set(cv_handle cvHandle, cv_fp_fe_control fpFeControl);
...
cv_status
cv_set_EC_presence(cv_handle cvHandle, cv_bool radioEnable);
...
cv_status
cv_set_system_id(cv_handle cvHandle, uint32_t systemID);
...
/* in */ uint32_t authListLen,				/* length of auth list in bytes */
...
cv_status
cv_enable_function ( 
/* in */ cv_function_enables functionEnable);

unsigned int cv_start_timer2(void);
unsigned int cv_get_timer2_elapsed_time(unsigned int prev);
