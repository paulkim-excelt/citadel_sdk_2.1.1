...
execmode_cmdline = 0xFFFE

toc_info = [
    # TOC 1
    TocInfo(toc_validity = 0xAA55AA55, media_cmd_len = 0, media_cmd = [0x1, 0x44307000, 0, 0, 0x2, 0x44407000, 0x12345678, 0, 3, 0x4459080c, 0xAA005500, 0xAA005500, 4, 0, 100, 0],
            sbi_info = [
                SbiInfo(file = "sbi.bin", offset = 0x760, power_mode = 0, exec_mode = execmode_cmdline),
            ])
]
...
def main(argv):
    ...
    # # Prefix all bin file names with bin dir path
    # i = 0
    # for file in bin_files:
        # bin_files[i] = os.path.join(bin_dir, file).strip()
        # i += 1

    # # Find file sizes
    # bin_file_sizes = get_file_info(bin_files)

    # i = 0
    # for file in bin_files:
        # print("File:%s Len:%d" % (bin_files[i], bin_file_sizes[i]))
        # i += 1
    ...
    # try:
        ...
        # # Create ToC
        # byte_array = create_toc(output, bin_files, bin_file_sizes)
        # #show_array(byte_array)

        # output.write(byte_array)
        # output.close()
    # except IOError as e:
        # error("File:\"" + der_file_name + "\": I/O error({0}): {1}".format(e.errno, e.strerror))
