...
/* Swaps the endianness of the input u32_t array and returns it in out array
 * pin can be pout for in-place endian swap
 */
...
int getch(void)
{
    int rx;
    u8_t buf[1];
    struct device *uart = device_get_binding(CONFIG_UART_NS16550_PORT_0_NAME);

    do {
        rx = uart_fifo_read(uart, buf, 1);
    } while (rx < 1);

    return buf[0];
}

static const uint32_t ctable[256] = {
