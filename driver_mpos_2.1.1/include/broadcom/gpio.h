...
/** Enable High speed slew rate. */
#define GPIO_SLEW_RATE_ENABLE		(1 << 16)

/** Enable hysteresis. */
#define GPIO_HYSTERESIS_ENABLE		(1 << 17)
...
typedef int (*gpio_config_t)(struct device *port, int access_op,
			     u32_t pin, int flags);
typedef int (*gpio_config_get_t)(struct device *port, u32_t pin, u32_t *flags);
...
typedef u32_t (*gpio_api_clr_pending_int)(struct device *dev, u32_t pin);

struct gpio_driver_api {
	gpio_config_t config;
	gpio_config_get_t config_get;
	gpio_write_t write;
	gpio_read_t read;
	...
	gpio_api_clr_pending_int clr_pending_int;
};
/**
 * @endcond
 */
...
/**
 * @brief Get the configure of single pin.
 * @param port Pointer to device structure for the driver instance.
 * @param pin Pin number to get the configuration.
 * @param flags Pointer to flags for pin configuration. IN/OUT, interrupt ...
 * @return 0 if successful, negative errno code on failure.
 */
static inline int gpio_pin_configure_get(struct device *port, u32_t pin,
					 u32_t *flags)
{
	const struct gpio_driver_api *api = port->driver_api;

	return api->config_get(port, pin, flags);
}
 ...
 * @return 0 if successful, negative errno code on failure
 */
static inline int gpio_clr_pending_int(struct device *dev, u32_t pin)
{
	struct gpio_driver_api *api;

    api = (struct gpio_driver_api *)dev->driver_api;
    return api->clr_pending_int(dev, pin);
}
