 ...
 * other definitions for the ARM Cortex-M processor architecture and
 * ARM Cortex-A architecture family.
 *
...
#error Unknown ARM architecture
	...
	__asm__ volatile(
		/* Switch to System mode (sp is shared with user mode) */
		"cps %[sys_mode]\n\t"
		/* Set SP_SYS, which is same as SP_USR */
		"mov sp, %[stack]\n\t"
		/* Unmask IRQ/FIQ/ABT exceptions */
		"cpsie ifa\n\t"
		: : [stack] "r" (start_of_main_stack),
		    [sys_mode] "i" (CPU_MODE_SYS)
	);
	z_thread_entry(_main, 0, 0, 0);
#endif
	CODE_UNREACHABLE;
