...
int mbedtls_gcm_self_test( int verbose )
{
    ...
    /* Output buffer should be block aligned */
    unsigned char buf[64]__attribute__ ((aligned (64)));
    unsigned char tag_buf[16]__attribute__ ((aligned (64)));
#else
...
#ifndef CONFIG_CRYPTO_BRCM_MBEDTLS
            /* Split data with unaligned block size is not supported */
            if( verbose != 0 )
                mbedtls_printf( "  AES-GCM-%3d #%d (%s): ",
                                key_len, i, "dec" );
