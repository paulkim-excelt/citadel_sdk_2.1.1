...
static inline void ecp_mpi_load( mbedtls_mpi *X, const mbedtls_mpi_uint *p, size_t len )
{
	...
	size_t cnt = len / sizeof( mbedtls_mpi_uint );
	/*
	 * Copy data from flash to ram. The implemenation doesn't support
	 * using flash address
	 */
	X->s = 1;
	mbedtls_mpi_grow(X, cnt);
	memcpy(X->p, p, len);
#else
    X->s = 1;
...
#ifdef CONFIG_CRYPTO_BRCM_MBEDTLS1
